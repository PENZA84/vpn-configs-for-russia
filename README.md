...
## TOPIC №1
**Если есть проблемы с доступом к подпискам GitHub через оригинальные ссылки** `https://raw.githubusercontent.com/igareck/` - **используйте альтернативно ссылки JSDelivr и GitHack через CDN** `https://cdn.jsdelivr.net/`, `https://rawcdn.githack.com/` и `https://raw.githack.com/`. 

**jsDelivr** - CDN для open-source файлов, в первую очередь GitHub/npm.

**GitHack** - caching proxy/CDN для raw-файлов из GitHub/GitLab/Bitbucket/Gitea/Codeberg.

**Альтернативные ссылки на каждую подписку расположены в разделе** `🪞 ЗЕРКАЛА 🪞`. **Менять в них ничего не нужно - просто замените ссылки у себя в клиенте и продолжайте пользоваться подпиской. В ближайшее время в дополнение к JSDelivr и GitHack выложу список разнообразных альтернатив.**
...
CDN для open-source файлов, в первую очередь GitHub/npm.

**3. Githack (через Cloudflare):** https://rawcdn.githack.com. 

Caching Proxy/CDN для raw-файлов из GitHub/GitLab/Bitbucket/Gitea/Codeberg.

**4. Githack (напрямую):** https://raw.githack.com

Пункты 2,3,4 помогут только в случае Черных Списков. Актуально, если Роскомнадзор заблокирует GitHub.
...
**СПОСОБ №2 и №3: `JSDelivr CDN` и `Githack Caching Proxy/CDN`**

Почему JSDelivr и Githack будут работать?
